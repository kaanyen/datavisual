...
def load_data():
    try:
        ...
        df = pd.read_csv('Final_merged_student_data.zip', low_memory=False, encoding='ISO-8859-1')
...
with tabs[0]:
    ...
    with colA:
        ...
        st.caption("This histogram shows the distribution of CGPAs across the entire active student body. Most students fall comfortably within the 2.5-3.5 range, well above the probation line (2.0).")
    ...
    with colB:
        ...
        fig_r.layout.yaxis.tickformat = ',.0%'
        st.plotly_chart(fig_r, use_container_width=True)
        st.caption("This bar chart identifies which demographic groups currently have the highest proportion of students at risk (CGPA < 2.0). Use the toggle to switch perspectives.")
    ...
    with st.expander("View At-Risk Student Roster (Filterable)", expanded=False):
        ...
        st.caption("A filtered list of currently active students who are below the 2.0 GPA threshold. This table is intended for academic advisors to prioritize outreach.")
...
with tabs[1]:
    ...
    with c1:
        ...
        st.caption("Shows the total number of new students admitted each year. The upward trend indicates university growth.")
    with c2:
        ...
        st.caption("Displays how many years students waited between finishing high school and applying. Most students apply within 1 year.")
    ...
    with c1:
        ...
        st.caption("Breakdown of student body by nationality groups.")
    with c2:
        ...
        st.caption("Percentage of students receiving financial aid vs. full-fee paying.")
    with c3:
        ...
        st.caption("Gender ratio across the entire student dataset.")
...
with tabs[2]:
    ...
    st.caption("A heatmap showing failure rates for the hardest courses across different majors. Red cells indicate high failure rates; white cells indicate low failure rates.")
    ...
    if not filtered_fail.empty:
        ...
        st.caption("Tracks failure rates as courses get more advanced (100 to 400 level), filtered only for successful graduates to show structural curriculum difficulty.")
    ...
    st.caption("A simple table ranking majors by average student CGPA. Darker green indicates higher average performance.")
    ...
    with c_gen:
        ...
        st.caption("Comparison of average semester GPA between male and female students over time.")
    with c_fin:
        ...
        st.caption("Compares the average cumulative GPA of Financial Aid students vs. Non-Aid students.")
    ...
    if not writ_course.empty:
        ...
        st.caption("Shows the average mark in a writing-intensive course grouped by the student's native language. Used to check for linguistic bias.")
    ...
    st.caption("Compares average student GPA across different pandemic phases. Note the slight dip during the lockdown/hybrid transition periods.")
...
with tabs[3]:
    ...
    st.caption("### **You can use the legend to toggle the different majors.**")
    ...
    if not switchers.empty:
        ...
        st.caption("Shows the most common paths students take when changing majors. Longer bars mean more students made that specific switch.")
    ...
    with c1:
        ...
        st.caption("A grid view of migration. Rows are starting majors, columns are ending majors. Dark squares indicate high movement.")
    
    with c2:
        st.subheader("3. Switch Success Predictor")
        st.caption("### **Pick a major to start and a major to switch to.**")
        ...
        if len(subset) > 0:
            ...
            st.caption("Calculates the historical success rate for students who made this specific switch.")
        else: st.info("No students found for this specific path.")

# -----------------------------------------------------------------------------
# TAB 5: GRADUATION HISTORY
# -----------------------------------------------------------------------------
with tabs[4]:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("Time to Degree")
        grads['Grad_Speed'] = grads['Years_To_Grad'].apply(lambda x: '4 Years' if x == 4 else ('< 4 Years' if x < 4 else '5+ Years'))
        grad_table = pd.crosstab(grads['Current_Major'], grads['Grad_Speed'], normalize='index') * 100
        st.dataframe(grad_table.style.format("{:.1f}%").background_gradient(cmap="Reds"))
        st.caption("Percentage of students graduating within 4 years vs. taking longer, broken down by major. Red indicates higher delay rates.")
    ...
    with c2:
        ...
        st.caption("Compares the average number of years it takes for Financial Aid vs. Non-Aid students to graduate.")

    st.subheader("Final Risk Summary (Count & %)")
    ...
    if not risk_groups.empty:
        ...
        st.caption("A detailed heatmap table showing the exact count and percentage of at-risk students for every major/nationality combination.")
...
with tabs[5]:
    ...
    st.caption("Ranking of factors that most strongly predict student risk. Positive values increase risk; negative values decrease it.")
